...
export function getInt64(buf: DataView, byteOffset: number = 0): JSBI {
    const i = getUint64(buf, byteOffset);
...
const CONVERTERS = new Map([
    ["f", {size: 4, read: (buf: DataView, off: number) => buf.getFloat32(off, LITTLE_ENDIAN), write: (v: any, buf: DataView, off: number) => buf.setFloat32(off, v, LITTLE_ENDIAN) }],
    ["d", {size: 8, read: (buf: DataView, off: number) => buf.getFloat64(off, LITTLE_ENDIAN), write: (v: any, buf: DataView, off: number) => buf.setFloat64(off, v, LITTLE_ENDIAN)}],
    ["c", {size: 1, read: (buf: DataView, off: number) => String.fromCharCode(buf.getUint8(off)), write: (v: any, buf: DataView, off: number) => buf.setUint8(off, v)}],
    ["b", {size: 1, read: (buf: DataView, off: number) => buf.getInt8(off), write: (v: any, buf: DataView, off: number) => buf.setInt8(off, v)}],
    ["B", {size: 1, read: (buf: DataView, off: number) => buf.getUint8(off), write: (v: any, buf: DataView, off: number) => buf.setUint8(off, v)}],
    ["h", {size: 2, read: (buf: DataView, off: number) => buf.getInt16(off, LITTLE_ENDIAN), write: (v: any, buf: DataView, off: number) => buf.setInt16(off, v, LITTLE_ENDIAN)}],
    ["H", {size: 2, read: (buf: DataView, off: number) => buf.getUint16(off, LITTLE_ENDIAN), write: (v: any, buf: DataView, off: number) => buf.setUint16(off, v, LITTLE_ENDIAN)}],
    ["i", {size: 4, read: (buf: DataView, off: number) => buf.getInt32(off, LITTLE_ENDIAN), write: (v: any, buf: DataView, off: number) => buf.setInt32(off, v, LITTLE_ENDIAN)}],
    ["I", {size: 4, read: (buf: DataView, off: number) => buf.getUint32(off, LITTLE_ENDIAN), write: (v: any, buf: DataView, off: number) => buf.setUint32(off, v, LITTLE_ENDIAN)}],
    ["q", {size: 8, read: (buf: DataView, off: number) => getInt64(buf, off), write: (v: any, buf: DataView, off: number) => setInt64(v, buf, off)}],
    ["Q", {size: 8, read: (buf: DataView, off: number) => getUint64(buf, off), write: (v: any, buf: DataView, off: number) => setUint64(v, buf, off)}],
]);
...
export function unpack(buf: DataView, typeString: string, fields: string[]): any {
    ...
    const o = {} as any;
    const wireTypes = parseWireTypeString(typeString);
    let bufIdx = 0;
    ...
    for(let i = 0; i < wireTypes.length; i++) {
        ...
        if (type.count > 1) {
            if (type.code === "c") {
                ...
                // Byte array
                const startIdx = bufIdx + buf.byteOffset;
                const endIdx = startIdx + type.count;
                o[field] = buf.buffer.slice(startIdx, endIdx);
                bufIdx += type.count;
            } else {
                // All other arrays
                o[field] = [];
                for(let j = 0; j < type.count; j++) {
                    o[field].push(converter.read(buf, bufIdx));
                    bufIdx += converter.size;
                }
            }
        } else {
            o[field] = converter.read(buf, bufIdx);
            bufIdx += converter.size;
        }
    }
    ...
    for(let i = 0; i < wireTypes.length; i++) {
        ...
        if (type.count > 1) {
            if (type.code === "c") {
                ...
                const bufView = new DataView(value);
                for (let j = 0; j < type.count; j++) {
                    buf.setUint8(byteOffset, bufView.getUint8(j));
                    byteOffset += 1;
                }
            } else {
                // All other arrays
                for (let j = 0; j < type.count; j++) {
                    const v = j < value.length ? value[j] : 0;

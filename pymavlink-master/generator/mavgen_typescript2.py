...
export interface IMavlinkPacket {
    ...
    for e in enums:
        ...
        for entry in e.entry:
            if entry.name.endswith('ENUM_END'):
                continue
            outf.write("    %-44s = %6u," % (entry.name, entry.value))
            if entry.description is not None and len(entry.description) > 0:
                outf.write("  %s" % element_comment_wrapper.fill(entry.description))
            outf.write("\n")
        outf.write("}\n\n")

    ...
    def __init__(self, filename, wire_protocol_version=PROTOCOL_0_9):
        ...
        for m in self.message:
            ...
            for i in range(len(m.ordered_fields)):
                ...
                f.set_test_value()
                if f.name.find('[') != -1:
                    raise MAVParseError("%s: invalid field name with array descriptor %s" % (m.name, f.name))
            ...
            if m.num_fields > 64:
                raise MAVParseError("%s: num_fields=%u : Maximum number of field names allowed is %u" % (
                    m.name, m.num_fields, 64))

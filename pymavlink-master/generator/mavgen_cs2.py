...
public partial class MAVLink
{
    ...
    static T[] parse_array<T>(byte[] buf, int offset, int wirelength, int elementSize)
    {
        var arr = new T[wirelength/elementSize];
        Buffer.BlockCopy(buf, offset, arr, 0, wirelength);
        return arr;
    }
    
    static byte[] serialize_array<T>(T[] elements, int wirelength, int elementSize)
    {
        var arr = new byte[wirelength];
        var count = Math.Min(elements.Length * elementSize, wirelength);
        Buffer.BlockCopy(elements, 0, arr, 0, count);
        return arr;
    }
    ...
    for m in messages:
        ...
        for f in m.fields:
            ...
            if is_string_field(f):
                ...
                if f.type == 'char':
                    f.type = "byte[]"
                    f.serialize =  'b.Write(serialize_array(v.{}, {}, 1));'.format(f.name, f.wire_length)
                    ...
                    f.type = "sbyte[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 1));'.format(f.name, f.wire_length)
                    f.deserialize = 'Array.ConvertAll(spanBuf.Slice({}, {}).ToArray(), Convert.ToSByte)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'int16_t':
                    f.type = "Int16[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 2));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<Int16>(buf, {}, {}, 2)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'uint16_t':
                    f.type = "UInt16[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 2));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<UInt16>(buf, {}, {}, 2)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'int32_t':
                    f.type = "Int32[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 4));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<Int32>(buf, {}, {}, 4)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'uint32_t':
                    f.type = "UInt32[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 4));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<UInt32>(buf, {}, {}, 4)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'long':
                    f.type = "Int64[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 8));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<Int64>(buf, {}, {}, 8)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'ulong':
                    f.type = "UInt64[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 8));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<UInt64>(buf, {}, {}, 8)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'float':
                    f.type = "float[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 4));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<float>(buf, {}, {}, 4)'.format(f.wire_offset, f.wire_length)

                elif f.type == 'double':
                    f.type = "double[]"
                    f.serialize = 'b.Write(serialize_array(v.{}, {}, 8));'.format(f.name, f.wire_length)
                    f.deserialize = 'parse_array<double>(buf, {}, {}, 8)'.format(f.wire_offset, f.wire_length)

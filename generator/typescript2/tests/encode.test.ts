...
test("MissionTransfer", () => {
    const pkt = mavlink.build_packet(mavlink.MSG_ID.ARC_MISSION_TRANSFER) as mavlink.IARC_MISSION_TRANSFER;
    
    const ID_BUF = new ArrayBuffer(16);
    let id = new DataView(ID_BUF);
    id.setUint8(0, 0x11);
    id.setUint8(1, 0xFF);
    id.setUint8(15, 0x99);
    pkt.mission_id = id.buffer;
    ...
    const m = messages[0] as mavlink.IARC_MISSION_TRANSFER;
    id = new DataView(m.mission_id);
    expect(id.getUint8(0)).toBe(0x11);
    expect(id.getUint8(1)).toBe(0xFF);
    expect(id.getUint8(15)).toBe(0x99);
});
...
const MAVLINK2_HEADER_LEN = 9;
const MAVLINK1_HEADER_LEN = 5;
const MAVLINK_NUM_HEADER_BYTES = (MAVLINK2_HEADER_LEN + 1);
...
const MAVLINK_MAX_PACKET_LEN = (MAVLINK_MAX_PAYLOAD_LEN + MAVLINK_NUM_NON_PAYLOAD_BYTES + MAVLINK_SIGNATURE_BLOCK_LEN);
...
function calculate_crc(buf: DataView, byteOffset: number, length: number, crc_extra: number): number {
    ...
    for (let i = 0; i < length; i++) {
        x = buf.getUint8(i + byteOffset) ^ (crc & 0xFF);
        x = (x ^ (x << 4)) & 0xFF;
        crc = (crc >> 8) ^ (x << 8) ^ (x << 3) ^ (x >> 4);
    }

    x = crc_extra ^ (crc & 0xFF);
    x = (x ^ (x << 4)) & 0xFF;
    crc = (crc >> 8) ^ (x << 8) ^ (x << 3) ^ (x >> 4);
    return crc;
}
...
export class Parser {
    ...
    ParseBytes(buf: ArrayBuffer): IMavlinkPacket[] {
        ...
        while(idx < parseBuf.length) {
            ...
            let msg: IMavlinkPacket | undefined;
            try {
    ...
    ParseSinglePacket(buf: DataView): IMavlinkPacket | undefined {
        ...
        const pkt = unpack(payloadView, pktDesc.format, pktDesc.fields);
        pkt.header = header;
        return pkt;
    }
}

export function encode(pkt: IMavlinkPacket): ArrayBuffer {

    const pktDesc = MAVLINK_MESSAGE_DESCRIPTORS.get(pkt.header.message_id);
    if (pktDesc == null) {
        throw new Error(`Unknown MAVLink message id: ${pkt.header}`);
    }
    ...
    buf.setUint8(4, pkt.header.sequence);               // sequence
    buf.setUint8(5, pkt.header.system_id);              // system id
    buf.setUint8(6, pkt.header.component_id);           // component id
    buf.setUint8(7, pkt.header.message_id & 0xFF);      // message id
    buf.setUint8(8, (pkt.header.message_id >> 8) & 0xFF);
    buf.setUint8(9, (pkt.header.message_id >> 16) & 0xFF);
...
export function build_packet(msg_id: MSG_ID): IMavlinkPacket {
